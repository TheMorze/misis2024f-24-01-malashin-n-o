...
void ArrayD::Resize(const std::ptrdiff_t size) {
  ...
  if (capacity_ < size) {
    ...
    if (size_ < size) {
      // если новый размер больше текущего, заполняем новые элементы нулями
...
void ArrayD::Insert(const std::ptrdiff_t idx, const double val) {
  ...
  Resize(size_ + 1);
  if (idx != Size() - 1) {
    // Сдвигаем элементы начиная с idx на 1 вперёд
...
void ArrayD::Remove(const std::ptrdiff_t idx) {
  ...
  if (idx != size_ - 1) {
    // Сдвигаем элементы начиная с idx на 1 назад
    std::memmove(data_ + idx, data_ + idx + 1, (size_ - idx - 1) * sizeof(double));
  }
  Resize(size_ - 1);
}
